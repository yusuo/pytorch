...
automatically saved as they are needed. You can explore (for educational or
debugging purposes) which tensors are saved by a certain ``grad_fn`` by looking
for its attributes starting with the prefix ``_saved``.
...
object than ``x`` (but they still share the same storage). You can control how
PyTorch does packing / unpacking with :ref:`saved-tensors-hooks-doc`.
...
Notice that the ``unpack_hook`` should not close / delete the temporary file
because it might be called multiple times: the temporary file should be alive
for as long as the returned `SelfClosingTempFile` object is alive.
In the above example, we prevent leaking the temporary file by closing it
when it is no longer needed (on deletion of the `SelfClosingTempFile` object).
...
.. code::
    ...
    def pack_hook(x):
        if x.numel() < SAVE_ON_DISK_THRESHOLD:
            return x
        sctf = SelfClosingTempFile()
        torch.save(x, sctf.fp)
        return sctf

    def unpack_hook(tensor_or_sctf):
        if isinstance(tensor_or_sctf, torch.Tensor):
            return tensor_or_sctf
        tensor_or_sctf.fp.seek(0)
        return torch.load(tensor_or_sctf.fp)
